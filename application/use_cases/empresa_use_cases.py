...
class ListarEmpresasUseCase:
    """Retorna todas as empresas em que o usuﾃ｡rio ﾃｩ membro."""
...
class CriarEmpresaUseCase:
    ...
    def executar(self, nome: str, descricao: str, usuario_id: int) -> Empresa:
        ...
        if hasattr(self, 'papel_repo') and self.papel_repo:
            ...
            self.papel_repo.salvar(Papel(None, empresa.id, "Member", "Acesso bﾃ｡sico ﾃ empresa", "ver_projetos,participar_projetos,criar_projetos"))

        return empresa


class EditarEmpresaUseCase:
    """Edita nome e descriﾃｧﾃ｣o de uma empresa (apenas owner/admin)."""
    ...
    def executar(self, empresa_id: int, nome: str, descricao: str, usuario_id: int) -> Empresa:
        ...
        if not empresa:
            raise ValueError("Empresa nﾃ｣o encontrada")
        ...
        if empresa.dono_id != usuario_id and (not membro or membro.role not in ('owner', 'admin')):
            raise PermissionError("Acesso negado: Vocﾃｪ nﾃ｣o possui permissﾃ｣o para realizar esta aﾃｧﾃ｣o.")
...
class RemoverMembroUseCase:
    ...
    def executar(self, empresa_id: int, usuario_id_alvo: int, usuario_id_solicitante: int) -> None:
        ...
        if empresa.dono_id == usuario_id_alvo:
            raise PermissionError("Acesso negado: Nﾃ｣o ﾃｩ possﾃｭvel remover o dono da empresa.")
        self.membro_repo.remover(empresa_id, usuario_id_alvo)


class SairEmpresaUseCase:
    """Usuﾃ｡rio remove a si mesmo da empresa.

    Regra: se for owner, sﾃｳ pode sair se existir pelo menos outro owner.
    """
    ...
    def executar(self, empresa_id: int, usuario_id: int) -> None:
        ...
        membro = self.membro_repo.buscar(empresa_id, usuario_id)
        if not membro and empresa.dono_id != usuario_id:
            raise PermissionError("Sem acesso ﾃ empresa")

        if empresa.dono_id == usuario_id:
            membros = self.membro_repo.listar_por_empresa(empresa_id)
            outros_owners = [
                m for m in membros
                if int(m.usuario_id) != int(usuario_id) and (m.role or "").strip().lower() == "owner"
            ]
            if len(outros_owners) < 1:
                raise PermissionError("Para sair, a empresa precisa ter pelo menos 1 outro Owner.")

            import random
            novo_dono = random.choice(outros_owners)
            empresa.dono_id = int(novo_dono.usuario_id)
            self.empresa_repo.atualizar(empresa)

        role = (membro.role or "").strip().lower() if membro else "member"
        if role == "owner":
            membros = self.membro_repo.listar_por_empresa(empresa_id)
            outros_owners = [
                m for m in membros
        ...
        self.membro_repo.remover(empresa_id, usuario_id)


class SolicitarAcessoUseCase:
    """Usuﾃ｡rio solicita entrada em uma empresa."""
    ...
    def executar(self, empresa_id: int, usuario_id: int, mensagem: str | None = None) -> Solicitacao:
        ja_membro = self.membro_repo.buscar(empresa_id, usuario_id)
        if ja_membro:
            raise ValueError("Vocﾃｪ jﾃ｡ ﾃｩ membro desta empresa")

        solicitacao = Solicitacao(id=None, empresa_id=empresa_id, usuario_id=usuario_id, mensagem=mensagem)
        return self.solicitacao_repo.salvar(solicitacao)


class GerenciarSolicitacaoUseCase:
    """Owner aprova ou recusa uma solicitaﾃｧﾃ｣o de acesso."""
...
class AlterarRoleMembroUseCase:
    ...
    def executar(self, empresa_id: int, usuario_id_alvo: int, novo_role: str, usuario_id_solicitante: int) -> Membro:
        if novo_role not in ("owner", "admin", "member"):
            raise ValueError("Role invﾃ｡lido")
        ...
        if is_admin and not is_owner and novo_role == 'owner':
            raise PermissionError("Acesso negado: Apenas o dono pode conceder privilﾃｩgios de Owner.")

        if empresa.dono_id == usuario_id_alvo:
            raise PermissionError("Acesso negado: Nﾃ｣o ﾃｩ possﾃｭvel alterar o cargo do dono da empresa.")

        membro_alvo = self.membro_repo.buscar(empresa_id, usuario_id_alvo)
        if not membro_alvo:
            raise ValueError("Membro nﾃ｣o encontrado")

        # Atualizando a entidade (precisa de db.commit no repository se formos usar atualizar, mas o repo atual nﾃ｣o tem atualizar_membro)
        # Como o MemberRepositoryImpl nﾃ｣o tem mﾃｩtodo "atualizar", vou deletar e adicionar para recriar com a nova role
        self.membro_repo.remover(empresa_id, usuario_id_alvo)
...
class EditarPapelUseCase:
    ...
    def executar(self, papel_id: int, nome: str, descricao: str, permissoes: str) -> Papel:
        ...
        if not papel: raise ValueError("Papel nﾃ｣o encontrado")
        papel.nome = nome
